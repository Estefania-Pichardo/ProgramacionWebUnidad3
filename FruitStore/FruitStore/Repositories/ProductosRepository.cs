...
namespace FruitStore.Repositories
{
    public class ProductosRepository:Repository<Productos>
    {
        ...
        public IEnumerable<Productos> GetProductosByCategoria(int? id)
        {
            return Context.Productos.Include(x=>x.IdCategoriaNavigation).Where(x => id==null|| x.IdCategoria == id).OrderBy(x=>x.Nombre);
        }
        ...
        public override bool Validate(Productos entidad)
        {
            ...
            {
                throw new Exception("Debe indicar la descripcion");
            }
